...
- [macronizer] The site's copy of the engine (`wiktionary_pron/wiktionary_pron/macronizer/dist/`) must be regenerated with `macronizer/sync-dist.sh` (build + rewrite the two host-specific asset paths), never hand-patched — the hand-patched copy had silently drifted from src. Its `.gitattributes` sets `* -text`: `macrons.txt` is served byte-for-byte and CRLF conversion would corrupt the plain-file fallback.
