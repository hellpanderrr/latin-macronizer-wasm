...
- [macronizer] Macronized INPUT text (ā, ī…) tears words apart downstream (fragments become "unknown" → Morpheus churn, ~10× slower) — strip combining U+0304/U+0306 after NFD before processing (UI layer does this; pipeline assumes unmarked input like Python).
- [macronizer] Heavy assets get fetched TWICE if the host page prefetches them: `WasmTagger`/`MorpheusAnalyzer` install their own Emscripten `locateFile` returning the network path, and `loadModel()` does its own `fetch(modelUrl)` — both ignoring a page-level blob cache. Route every asset URL through `src/utils/assets.ts: resolveAssetUrl(path, fallback)` (checks `window.__fileCacheUrls` by basename). Cost of the bug: ~35 MB of duplicate transfer on first visit.
- [macronizer] `Statistics.knownWords/unknownWords` means "lemma OR ending-pattern match" — a DIFFERENT notion from `Token.isUnknown` ("not in the wordlist"). A UI that highlights `isUnknown` words but prints `statistics.unknownWords` will contradict itself (reported 2 unknowns with 0 words marked). Derive user-facing counts from the tokens you actually render. Same for `confidence`: it's a coverage heuristic (0.95/0.85/0.60 per token), NOT a probability the macrons are right — don't label it "Confidence" in a UI.
- [macronizer] `EndingPatternEngine.hasPattern()` was O(all endings) with a fresh regex per ending, called once per token by the stats pass — 40% of total runtime on a 2k-word text (13.2s → 5.5s once precomputed into a suffix Set). Watch for per-token calls into engines that scan their whole table.
- [macronizer] A blob/prefetch cache in front of the engine HIDES broken asset paths: `resolveAssetUrl(path, fallback)` returns the cached blob before the fallback is ever used, so a 404-ing fallback looks fine until the cache misses. `WasmTagger` fetched the model from a hardcoded `/wasm/rftagger-ldt.model` for exactly this reason (Macronizer passed the real path as `modelPath`, a field that was stored and never read; `loadModel()` reads `modelUrl`). When a host serves the engine from another directory, verify the *fallback* URLs resolve — don't trust that the page loads.
- [macronizer] The site (hellpanderrr.github.io) carries a COMPILED COPY of the engine at `wiktionary_pron/macronizer/dist/`. `npm run build` regenerates it automatically (`sync-site.cjs`, skips silently if the site checkout is absent; override with `MACRONIZER_SITE_DIR`). NEVER hand-edit that copy: it was hand-patched to fix asset paths, which hid a wrong path in `src/` until a rebuild exposed it. The site's `.gitattributes` sets `* -text` — `macrons.txt` is served byte-for-byte and CRLF conversion would corrupt the plain-file fallback.
