 ...
 * Usage:
 *   node scripts/doref.mjs <ref.txt>          # Run CLI, compare against ref file
 *   node scripts/doref.mjs <ref.txt> <out.txt> # Same but read out file instead of running
 */
...
async function main() {
  ...
  if (!refFile) { console.error('Usage: node scripts/doref.mjs <ref.txt> [out.txt]'); process.exit(1); }
