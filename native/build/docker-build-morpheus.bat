...
docker-compose -f "%PROJECT_ROOT%\native\morpheus\js\docker-compose.morpheus.yml" run --rm morpheus-wasm-builder
...
if /i "%START_SERVER%"=="Y" (
    ...
    echo.
    docker-compose -f "%PROJECT_ROOT%\native\morpheus\js\docker-compose.morpheus.yml" up morpheus-server
) else (
    echo.
    echo To test later, run: docker-compose -f native/morpheus/js/docker-compose.morpheus.yml up morpheus-server
    echo.
