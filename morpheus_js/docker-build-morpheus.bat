...
REM Navigate to project root (assumes script is in morpheus_js/)
cd /d "%~dp0.."
...
docker-compose -f morpheus_js/docker-compose.morpheus.yml build --no-cache morpheus-wasm-builder
...
docker-compose -f morpheus_js\docker-compose.morpheus.yml run --rm ^
    --no-deps ^
...
echo.
echo To run test server: docker-compose -f morpheus_js/docker-compose.morpheus.yml up morpheus-server
