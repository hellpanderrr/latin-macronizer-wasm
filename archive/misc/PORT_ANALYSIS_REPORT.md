...
**Directories analyzed:** `src/` (port), `dist/`, `morpheus_js/`, `latin_macronizer/` (original)
...
**Python:** Calls external `cruncher` (C code from `morpheus-master/`)
...
- Also separate implementation in `morpheus_js/`: `MorpheusTagger.ts`, `morpheus_wrapper.c`
...
| `morpheus_wrapper.c` | `morpheus_js/` | ✅ Wrapper source code |
| `MorpheusTagger.ts` | `morpheus_js/` | ✅ TypeScript wrapper |
...
**Python comparison:**
- `morpheus_js/compare_results.py` - result comparison script
- `morpheus_js/test-compare.bat` - comparison batch file
