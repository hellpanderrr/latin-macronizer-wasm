...
export class Tokenization {
  ...
  private macronizeToken(
    ...
    if (!domacronize) {
      if (shouldLogAlign) console.log(`  domacronize=false, returning early`);
  ...
  detokenize(markAmbigs: boolean = false): string {
    ...
    for (const token of this.tokens) {
      ...
      // Add token text
      const text = (token as any).macronizedText ?? token.text;
      
      if (markAmbigs && (token as any).isAmbiguous) {
        result += `<span class="ambig">${text}</span>`;
      } else if ((token as any).isUnknown) {
        result += `<span class="unknown">${text}</span>`;
      } else {
        result += text;
      }
      
      lastEnd = (token as any).endIndex ?? (start + text.length);
    }
