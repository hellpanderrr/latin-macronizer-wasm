...
(async () => {
    ...
    page.on('request', request => {
        if (request.url().endsWith('macrons.txt')) {
            request.respond({
                status: 200,
                contentType: 'text/plain',
                body: 'Gallia	gallia	N	nom	sg	fem\n'
            });
        } else {
            request.continue();
        }
    ...
    console.log('Clicking "Initialize Macronizer"...');
    ...
    const inputText = 'Gallia est omnis divisa in partes tres';
