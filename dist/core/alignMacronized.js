...
export function alignMacronized(plain, accented, options = {}) {
    ...
    if (!domacronize && !performutov && !performitoj) {
        if (shouldLog)
            console.log('    Early exit: no macronization/conversion needed');
        return plain;
    }
    ...
    if (isExactMatch) {
        ...
        if (shouldLog)
            console.log(`    Early exact match: "${plain}" === "${accentedWithoutUnderscores}" (alsomaius=${options.alsomaius}), returning "${result}"`);
        return result;
    }
    ...
    while (i > 0 || j > 0) {
        ...
        if (move === 'diag' && i > 0 && j > 0) {
            ...
            if (shouldLog)
                console.log('      Safety break: stuck in backtrack');
            break;
        }
    }
