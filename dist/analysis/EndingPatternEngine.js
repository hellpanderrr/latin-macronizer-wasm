...
export class EndingPatternEngine {
    constructor() {
        ...
        Object.defineProperty(this, "suffixTree", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    apply(word, posTag) {
        ...
        const sortedPatterns = [...this.patterns].sort((a, b) => (b.priority || 0) - (a.priority || 0));
    ...
    inferTag(word) {
        ...
        if (lowerWord.endsWith('um'))
            return 'n-s--n-';
        if (lowerWord.endsWith('a') && !lowerWord.endsWith('ia'))
            return 'n-s--f-';
        ...
        for (const pattern of this.patterns) {
            if (lowerWord.endsWith(pattern.suffix)) {
                if (!posTag || !pattern.posTags || pattern.posTags.length === 0) {
                    ...
                    const matchesPos = pattern.posTags.some(tag => tag.charAt(0) === mainPos);
                    if (matchesPos) {
                        matches.push(pattern);
                    }
                }
     ...
     */
    macronizeWithPatterns(word, posTag) {
